...
"""Docker e2e failure tests for SLAE REST v1 Web Server-to-Worker flow."""
...
_WORKER_ID = "slae-worker-rest-v1-failure-local"
...
_MATRIX_CSV = "2,1\n1,3\n"
_RHS_CSV = "1\n2\n"
_SINGULAR_MATRIX_CSV = "0,0\n0,0\n"
_ZERO_RHS_CSV = "0\n0\n"

_SINGULAR_MATRIX_ERROR_MESSAGE_FRAGMENTS = (
    "singular",
    "determinant",
    "linearly dependent",
    "linear dependent",
...
_ERROR_HANDLER_MESSAGE_FRAGMENTS = (
    "synthetic division by zero",
    "division by zero",
)
...
_JOB_ERROR_TIMEOUT_SECONDS = 60.0
...
def test_slae_rest_v1_web_server_to_worker_flow_reports_error_for_singular_matrix() -> (
    None
):
    """Verify REST v1 flow reports ERROR for singular matrix execution failure."""

    _run_web_server_to_worker_failure_flow(
        upload_session_prefix="singular-matrix-failure-session",
        worker_handler=_SLAE_JOB_HANDLER,
        matrix_csv=_SINGULAR_MATRIX_CSV,
        rhs_csv=_ZERO_RHS_CSV,
        solving_method=_SOLVING_METHOD,
        expected_error_message_fragments=_SINGULAR_MATRIX_ERROR_MESSAGE_FRAGMENTS,
    )


def test_slae_rest_v1_web_server_to_worker_flow_reports_error_when_handler_fails() -> (
    None
):
    """Verify REST v1 flow reports ERROR for supervised handler failure."""

    _run_web_server_to_worker_failure_flow(
        upload_session_prefix="handler-failure-session",
        worker_handler=_ERROR_JOB_HANDLER,
        matrix_csv=_MATRIX_CSV,
        rhs_csv=_RHS_CSV,
        solving_method=_SOLVING_METHOD,
        expected_error_message_fragments=_ERROR_HANDLER_MESSAGE_FRAGMENTS,
    )


def _run_web_server_to_worker_failure_flow(
    *,
                    ...
                    with _new_minio_container(e2e_network) as minio_container:
                        ...
                        with _new_web_server_container(
                            ...
                            with _new_slae_worker_container(
                                ...
                                _run_failure_job_flow(
                                    e2e_environment=e2e_environment,
                                    upload_session_prefix=upload_session_prefix,
...
def _run_failure_job_flow(
    *,
    ...
    error_status = _wait_for_job_error(
        web_server_url=web_server_url,
        job_id=job_id,
    )

    assert str(error_status["jobId"]) == job_id

    if "jobType" in error_status:
        assert error_status["jobType"] == _JOB_TYPE
    ...
    assert any(
        ...
        "ERROR message does not contain expected failure details. "
        f"message={error_message!r}, "
        f"expectedFragments={expected_error_message_fragments!r}"
    )

    _assert_job_stays_error(
        web_server_url=web_server_url,
        job_id=job_id,
...
def _wait_for_job_error(
    *,
    web_server_url: str,
    job_id: str,
) -> dict[str, Any]:
    last_status: dict[str, Any] | None = None
    deadline = time.monotonic() + _JOB_ERROR_TIMEOUT_SECONDS
    ...
    while time.monotonic() < deadline:
        ...
        if status_name in {"DONE", "CANCELLED"}:
            raise AssertionError(
    ...
    raise AssertionError(
        "Job did not reach ERROR before timeout: "
        f"jobId={job_id!r}, lastStatus={last_status!r}"
    )


def _assert_job_stays_error(
    *,
    web_server_url: str,
    job_id: str,
    ...
    while time.monotonic() < deadline:
        ...
        assert str(status.get("status", "")) == "ERROR"
