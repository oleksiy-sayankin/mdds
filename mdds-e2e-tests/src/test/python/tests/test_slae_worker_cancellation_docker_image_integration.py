...
"""Cancellation e2e tests for SLAE Python Worker Docker image."""
...
_WORKER_ID = "slae-worker-cancellation-local"
...
@dataclass(frozen=True)
class SlaeCancellationWorkerCluster:
    rabbitmq: RabbitMqEndpoint
    s3_client: Any
    worker_health_url: str


def test_slae_worker_docker_image_publishes_cancelled_when_running_job_is_cancelled() -> (
    None
):
    """Verify Dockerized SLAE worker cancels a running supervised job.
    ...
    Docker image
      ...
      -> CANCELLED
      -> submitted/cancellation message acknowledgement
    """
    test_id = str(uuid4())

    job_queue_name = f"mdds.local.job.solving_slae.cancellation.{test_id}"
    status_queue_name = f"mdds.local.status.solving_slae.cancellation.{test_id}"
    cancel_queue_name = f"mdds.local.cancel.solving_slae.cancellation.{test_id}"
    ...
    job_id = f"job-cancellation-{test_id}"
        ...
        with _new_rabbitmq_container(e2e_network) as rabbitmq_container:
            ...
            try:
                with _new_minio_container(e2e_network) as minio_container:
                    ...
                    with _new_hanging_slae_worker_container(
                        ...
                        with StatusQueueConsumer(
                            ...
                            _wait_until(
                                ...
                                is None,
                                description=(
                                    "Cancellation queue to become empty after "
                                    "cancellation ack"
                                ),
                                timeout_seconds=_QUEUE_DRAIN_TIMEOUT_SECONDS,
                            )
...
def test_slae_worker_docker_image_ignores_duplicate_cancel_after_cancelled() -> None:
    """Verify duplicate cancellation does not publish a second CANCELLED status."""
    test_id = str(uuid4())

    job_queue_name = f"mdds.local.job.solving_slae.duplicate.cancel.{test_id}"
    status_queue_name = f"mdds.local.status.solving_slae.duplicate.cancel.{test_id}"
    cancel_queue_name = f"mdds.local.cancel.solving_slae.duplicate.cancel.{test_id}"
    ...
    job_id = f"job-duplicate-cancel-{test_id}"
                ...
                with _new_minio_container(e2e_network) as minio_container:
                    ...
                    with _new_hanging_slae_worker_container(
                        ...
                        with StatusQueueConsumer(
                            ...
                            first_cancelled_status = _wait_for_job_status(
                                status_consumer=status_consumer,
                                job_id=job_id,
                                expected_status="CANCELLED",
                                received_statuses=statuses,
                            ...
                            statuses_after_duplicate_cancel = (
                                _collect_statuses_for_observation_window(
                                    status_consumer=status_consumer,
                                    observation_seconds=(
                                        _POST_TERMINAL_OBSERVATION_SECONDS
                            ...
                            statuses.extend(statuses_after_duplicate_cancel)
                            statuses.extend(status_consumer.drain())
                            ...
                            assert (
                                _terminal_statuses_for_job(
                                    statuses=statuses_after_duplicate_cancel,
                                    job_id=job_id,
                                )
                                == []
                            )
...
def _new_hanging_slae_worker_container(
    *,
    e2e_network: Network,
    job_queue_name: str,
