...
"""Docker e2e timeout tests for SLAE REST v1 Web Server-to-Worker flow."""
...
_WORKER_ID = "slae-worker-rest-v1-timeout-local"
...
def e2e_environment() -> Iterator[E2eEnvironment]:
    """Start shared Docker infrastructure for the REST v1 timeout scenario."""
...
def test_slae_rest_v1_web_server_to_worker_flow_reports_error_when_job_times_out(
    e2e_environment: E2eEnvironment,
) -> None:
    """Verify REST v1 flow reports ERROR when SLAE Worker job times out."""

    _run_timeout_job_flow(e2e_environment=e2e_environment)


def _run_timeout_job_flow(*, e2e_environment: E2eEnvironment) -> None:
    upload_session_id = f"timeout-session-{uuid4()}"
...
def _new_timeout_slae_worker_container(e2e_network: Network) -> DockerContainer:
    return (
