...
def test_context_factory_rejects_null_manifest(tmp_path) -> None:
    jobs_root = tmp_path / "jobs"
    prepared_job_inputs = _create_prepared_job_inputs(jobs_root)
...
def test_context_factory_rejects_null_prepared_inputs(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="prepared_inputs cannot be null."):
        JobExecutionContextFactory(jobs_root).create(
            manifest,
            None,
        )
...
def test_context_factory_rejects_mismatched_user_id(tmp_path) -> None:
    jobs_root = tmp_path / "jobs"
    manifest = _create_manifest(user_id=42)
    prepared_job_inputs = _create_prepared_job_inputs(jobs_root, user_id=43)

    with pytest.raises(
        ValueError,
        match="prepared inputs user_id does not match manifest user_id.",
    ):
        JobExecutionContextFactory(jobs_root).create(
            manifest,
            prepared_job_inputs,
...
def test_context_factory_rejects_mismatched_job_id(tmp_path) -> None:
    jobs_root = tmp_path / "jobs"
    manifest = _create_manifest(job_id="job-1")
    prepared_job_inputs = _create_prepared_job_inputs(jobs_root, job_id="job-2")

    with pytest.raises(
        ValueError,
        match="prepared inputs job_id does not match manifest job_id.",
    ):
        JobExecutionContextFactory(jobs_root).create(
            manifest,
            prepared_job_inputs,
...
def _create_manifest(
    ...
    outputs: dict[str, SimpleNamespace] | None = None,
