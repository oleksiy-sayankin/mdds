...
class CleanupWatcher:
    """Removes cleanup-ready execution records and local job workspaces.
    ...
    A record is cleanup-eligible only after terminal status publication and
    ...
    def poll_once(self) -> None:
        """Process one snapshot of cleanup-ready execution records.
    ...
    def _process_record(self, record: ExecutionRecord | None) -> None:
        ...
        try:
            ...
            self._remove_execution_record(record)
        except Exception:
            logger.exception(
                "Execution record cleanup failed; record will remain cleanup-ready for retry.",
                extra={
                    "component": "cleanup_watcher",
                    ...
                    "localPath": str(record.context.work_dir),
                },
            )

    @staticmethod
    def _is_cleanup_candidate(record: ExecutionRecord) -> bool:
        with record.lock:
            return (
                record.terminal_status_claimed
                and record.terminal_status_published
                and record.acknowledgement_done
                and record.cleanup_ready
            )

    def _delete_local_workspace(self, record: ExecutionRecord) -> None:
        work_dir = Path(record.context.work_dir)
    ...
    def _remove_execution_record(self, record: ExecutionRecord) -> None:
        removed = self._execution_registry.remove_if_same(record)

        if not removed:
            logger.warning(
                "Cleanup-ready execution record was not removed because registry entry changed or disappeared.",
                extra={
                    "component": "cleanup_watcher",
                    "event": "cleanup_ready_record_remove_skipped",
                    "jobId": record.job_id,
                    "userId": record.user_id,
                    "jobType": record.job_type,
                    "workerId": record.worker_id,
        ...
        logger.info(
            "Cleanup-ready execution record removed.",
            extra={
                "component": "cleanup_watcher",
                "event": "cleanup_ready_record_removed",
                "jobId": record.job_id,
                "userId": record.user_id,
                "jobType": record.job_type,
                "workerId": record.worker_id,
